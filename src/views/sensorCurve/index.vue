    ...
    <div id="sensorCurve">
      <el-col class="section senCurve-section">
        <div class="section-title senCurve-title">
          <el-col :span="11" class="senCurve-title-left">
            <svg-icon class="svg-logo" icon-class="" />
            <span>温湿度历史曲线</span>
          </el-col>
          <el-col :span="5" class="senCurve-title-right">
            <el-date-picker :picker-options="pickerOptions" v-model="value2" align="right" type="date" placeholder="选择日期"/>
          </el-col>
        </div>
        <div class="section-info senCurve-info">
          <div id="senCurve" :style="{width: '100%', height: '300px'}"/>
        </div>
      </el-col>
    </div>
  </div>
...
//  name: 'sensorCurve',
  ...
  methods: {
    curveInfo() {
      const senCurve = echarts.init(document.getElementById('senCurve'))
      senCurve.setOption({
        title: {
          text: '温/湿度曲线',
          textStyle: {
